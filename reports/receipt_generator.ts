...
const BRAND = {
  headerBg:   [124, 58, 237] as [number, number, number],
  sectionBg:  [22, 163, 74]  as [number, number, number],
  ...
  amountGreen:[22, 163, 74]  as [number, number, number],
  stampGreen: [22, 163, 74]  as [number, number, number],
};
...
export const generateReceiptImage = async (data: ReceiptGeneratorData) => {
  ...
  // ── Header ──────────────────────────────────────────────────────────────
  fillRoundRect(ctx, M - 4, M - 4, CW + 8, 82, 6, '#7c3aed');
  ...
  const drawSection = (title: string, rows: [string, string, boolean?][]) => {
    ...
    rows.forEach(([label, value, highlight], i) => {
      ...
      ctx.fillStyle = highlight ? '#16a34a' : '#0a1628';
