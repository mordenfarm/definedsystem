    ...
    <div className="space-y-5">
      <header className="flex items-center justify-between">
        ...
        <button
          ...
          className="h-11 w-11 rounded-full bg-white shadow-lg grid place-items-center text-[#7c3aed] disabled:opacity-40"
        >
          <Download size={17} />
        </button>
      </header>

      <section className="rounded-[30px] bg-gradient-to-br from-[#7c3aed] via-[#9f5cff] to-[#ff8f2f] p-5 text-white shadow-[0_20px_50px_rgba(124,58,237,0.28)] overflow-hidden relative">
      ...
      <section className="rounded-[30px] bg-white p-4 shadow-[0_18px_45px_rgba(76,29,149,0.12)] space-y-4">
          ...
          {['Term 1', 'Term 2', 'Term 3'].map(term => (
            <button
              ...
              className={`h-10 rounded-2xl text-[10px] font-black transition-all ${
                selectedTerm === term ? 'bg-[#7c3aed] text-white shadow-lg shadow-purple-200' : 'bg-[#f7f1ff] text-slate-500'
              }`}
            >
              {term}
            </button>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-2 rounded-[22px] bg-[#fbf8ff] px-4 py-3 border border-[#eadcff]">
        ...
        <div className="grid grid-cols-1 gap-2">
          <button
            ...
            className="h-12 rounded-[24px] bg-[#7c3aed] text-white text-[11px] font-black flex items-center justify-center gap-2 shadow-[0_14px_30px_rgba(124,58,237,0.28)] disabled:opacity-45 transition-all active:scale-[0.98]"
          >
          ...
          <button
            ...
            disabled={isProcessing || !validateAmount()}
            className="h-12 rounded-[24px] bg-[#f2e8ff] text-[#7c3aed] text-[11px] font-black flex items-center justify-center gap-2 disabled:opacity-45 transition-all active:scale-[0.98]"
          >
            <ShieldCheck size={15} />
            Auto system receipt
          </button>
      ...
      <section className="rounded-[30px] bg-white p-4 shadow-[0_18px_45px_rgba(76,29,149,0.12)]">
        ...
        <div className="space-y-2">
          {studentPayments.length === 0 ? (
            <div className="rounded-2xl bg-[#fbf8ff] p-4 text-[10px] font-bold text-slate-400">No payment records yet.</div>
          ) : studentPayments.slice(0, 5).map(payment => (
            <div key={payment.id} className="rounded-2xl bg-[#fbf8ff] px-3 py-3 flex items-center gap-3">
