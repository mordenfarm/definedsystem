...
const FrontFace: React.FC<{
  ...
  expiryDate: string;
  qrDataUrl: string;
}> = ({ student, issueDate, expiryDate, qrDataUrl }) => (
  <>
    <div className="absolute -right-10 top-32 h-[360px] w-[360px] opacity-[0.04] pointer-events-none">
  ...
  <>
    <header className="relative flex h-[104px] items-center justify-between overflow-hidden bg-gradient-to-r from-[#16052f] via-[#3b0a72] to-[#6d28d9] px-8">
      ...
      <div className="flex items-center gap-4">
        <div className="grid h-12 w-12 place-items-center rounded-full border-2 border-emerald-500 bg-white p-1.5 shadow-md">
          <img src={LogoImg} alt="Defined Domains" crossOrigin="anonymous" />
        </div>
        <div>
          <h3 className="text-[22px] font-black uppercase leading-none tracking-wider text-white">Defined Domains</h3>
          <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#4ade80]">Terms of use & student record</p>
        </div>
      </div>
  ...
  if (forceStatic) {
    return (
      <div ref={ref} className="relative h-[540px] w-[856px] overflow-hidden rounded-[22px] border-2 border-slate-200 bg-white font-sans shadow-2xl">
        {!showingBack ? (
          <FrontFace student={student} issueDate={issueDate} expiryDate={expiryDate} qrDataUrl={qrDataUrl} />
        ) : (
          <BackFace student={student} qrDataUrl={qrDataUrl} />
        )}
      </div>
    );
  }

  // Interactive 3D flip card with smooth horizontal rotation
  return (
    <div ref={ref} className="h-[540px] w-[856px] [perspective:1400px] select-none">
      <div
        className={`relative h-full w-full rounded-[22px] transition-transform duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] [transform-style:preserve-3d] ${
          showingBack ? '[transform:rotateY(180deg)]' : '[transform:rotateY(0deg)]'
        }`}
      >
        {/* Front Face */}
        <div className="absolute inset-0 h-full w-full overflow-hidden rounded-[22px] border-2 border-slate-200 bg-white font-sans shadow-2xl [backface-visibility:hidden]">
          <FrontFace student={student} issueDate={issueDate} expiryDate={expiryDate} qrDataUrl={qrDataUrl} />
        </div>

        {/* Back Face */}
        <div className="absolute inset-0 h-full w-full overflow-hidden rounded-[22px] border-2 border-slate-200 bg-white font-sans shadow-2xl [transform:rotateY(180deg)] [backface-visibility:hidden]">
          <BackFace student={student} qrDataUrl={qrDataUrl} />
        </div>
      </div>
    </div>
